...
import {FormUserRole, UserFormProps, ValidationErrors } from '../../../types/user';
...
export function UserForm({
  ...
  const validateIFSC = (ifsc: string): boolean => {
    // IFSC format: 4 letters + 0 + 6 characters (letters/numbers)
  ...
  const validateField = (fieldName: keyof ValidationErrors, value: string): string => {
    switch (fieldName) {
      case 'name':
        ...
        if (!validateName(value)) return 'Name should contain only letters and spaces (2-50 characters)';
        return '';
      ...
      case 'mobile':
        ...
        if (!validateMobileNumber(value)) return 'Mobile number must be 10 digits starting with 6-9';
        return '';
      ...
      case 'password':
        ...
        if (value && !validatePassword(value)) return 'Password must be at least 8 characters with letters and numbers';
        return '';

      case 'bankAccount':
        if (!value.trim()) return '';
        if (!validateBankAccount(value)) return 'Bank account number must be 9-18 digits';
        return '';

      case 'ifsc':
        if (!value.trim()) return '';
        if (!validateIFSC(value)) return 'IFSC code format: ABCD0123456 (4 letters + 0 + 6 characters)';
        return '';
  ...
  const handleFieldChange = (fieldName: keyof ValidationErrors, value: string) => {
  ...
  const validateAllFields = (): boolean => {
    ...
    // Bank details validation for specific roles
    if (userData.role === 'Zonal Head' || userData.role === 'State Head') {
      if (userData.bankAccount) {
        const bankAccountError = validateField('bankAccount', userData.bankAccount);
        if (bankAccountError) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-[#0066cc] rounded-t-xl sticky top-0">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={`mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white ${
                validationErrors.mobile ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
              }`}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => onUserDataChange({ ...userData, addressLine1: e.target.value })}
              className="mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white border-gray-300 dark:border-gray-600"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => onUserDataChange({ ...userData, addressLine2: e.target.value })}
              className="mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white border-gray-300 dark:border-gray-600"
          ...
          <div className="relative" ref={stateDropdownRef}>
            ...
            <div className="relative mt-1">
              <input
                type="text"
                value={stateDropdownOpen ? stateSearchTerm : (userData.state || '')}
              ...
              <button
                ...
                className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-600 dark:text-gray-400"
              >
                <ChevronDown className={`h-5 w-5 transform transition-transform ${stateDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              {stateDropdownOpen && (
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                  {filteredStates.length > 0 ? (
                    ...
                    <div className="px-3 py-2 text-gray-500 dark:text-gray-400">No states found</div>
                  )}
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => onUserDataChange({ ...userData, district: e.target.value })}
              className="mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white border-gray-300 dark:border-gray-600"
          ...
          <div>
            ...
            <input
              ...
              className={`mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white ${
                validationErrors.pincode ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
              }`}
          ...
          {/* Plant Location field for Printer Users only */}
            ...
            <div className="col-span-2">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Plant Location</label>
                <select
                value={userData.plantLocation || ''}
                onChange={(e) => onUserDataChange({ ...userData, plantLocation: e.target.value })}
                className="mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white border-gray-300 dark:border-gray-600"
                >
          ...
          {(userData.role === 'Zonal Head' || userData.role === 'State Head') && (
            <div className="col-span-2 border border-gray-400 rounded-lg p-4">
              <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{t.bankDetails}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => onUserDataChange({ ...userData, bankName: e.target.value })}
                    className="mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white border-gray-300 dark:border-gray-600"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => onUserDataChange({ ...userData, accountHolderName: e.target.value })}
                    className="mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white border-gray-300 dark:border-gray-600"
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white ${
                      validationErrors.bankAccount ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                    }`}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`mt-1 w-full p-2 border rounded-lg dark:bg-gray-700 dark:text-white ${
                      validationErrors.ifsc ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                    }`}
                    placeholder="ABCD0123456"
                  />
                  {validationErrors.ifsc && (
                    <p className="mt-1 text-sm text-red-500">{validationErrors.ifsc}</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-end space-x-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 sticky bottom-0 bg-white dark:bg-gray-800">
          ...
          <button 
            ...
            disabled={isLoading}
            className={`px-4 py-2 ${isLoading ? 'bg-gray-400' : 'bg-[#0066cc] hover:bg-blue-700'} text-white rounded-lg`}
          >
            {isLoading 
              ? (isUpdate ? 'Updating...' : 'Saving...') 
              : (isUpdate ? t.updateUserBtn : t.saveUser)
            }
          </button>
        </div>
      </div>
